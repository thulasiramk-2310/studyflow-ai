# Look up AWS-managed CloudFront policies by name (IDs vary / are brittle).
...
# Private S3 bucket holding the built React app. Only CloudFront can read it.
...
resource "aws_cloudfront_distribution" "site" {
  ...
  # Backend API/auth, via the ALB (HTTP within AWS; viewer->CloudFront is HTTPS).
  origin {
...
# Allow the CloudFront distribution to read objects from the bucket.
